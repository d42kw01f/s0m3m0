...
class Comment(Post):
    def __init__(self, data):
        ...
        self.comment_sentiment_score = data.get('pt_the_senti', {}).get('sentiment_score', 0)
        self.comment_candidate_scores = data.get('pt_the_candi', {})


class EngagementAnalyzer:
    def __init__(self, post_data):
        # Initialize with a single post by wrapping it in a list
    ...
    def calculate_post_weights(self):
        ...
        for i, post in enumerate(self.posts):
            ...
            for e_candidate in self.candidates:
                ...
                self.total_candidate_weights[e_candidate] += (
                    E_p * C_p_c * post.sentiment_score * post.decay_factor
                )
            ...
            for j, comment in enumerate(post.top_comments):
                ...
                for e_candidate in self.candidates:
                    C_c_c = comment.comment_candidate_scores.get(e_candidate, 0)
                    self.total_candidate_weights[e_candidate] += (
                        E_c * C_c_c * comment.comment_sentiment_score * comment.decay_factor
                    )
    ...
    def get_normalized_candidate_weights(self):
        ...
        normalized_weights = {
            m_candidates: (m_weight / sum_of_abs_weights) * 100 if sum_of_abs_weights else 0
            for m_candidates, m_weight in self.total_candidate_weights.items()
        }
...
if __name__ == '__main__':
    # Example usage with a single JSON object
    post_data = {
        'post_id': 1,
        'platform': 'Facebook',
        'reactions_count': {
        ...
        'comment_count': 80,
        'share_count': 40,
        'publishedAt': 'Fri, 20 Sep 2024 12:58:32 GMT+0000',  # Updated date format
    ...
    analyzer = EngagementAnalyzer(post_data)
